...
import { getTodayString, getNextWeekSaturday } from '../../../utils/dateUtils';
...
export const SpaceReservationForm: React.FC<SpaceReservationFormProps> = ({
  ...
  const applicant: ApplicantProfile = {
    name: pageData?.applicantName || propApplicant?.name || '이름',
    studentId: pageData?.applicantStudentId || propApplicant?.studentId || '학번',
    phone: pageData?.applicantPhone || propApplicant?.phone || '010-0000-0000',
    email: pageData?.applicantEmail || propApplicant?.email || 'student@hansung.ac.kr',
  };
  // 오늘 날짜를 기본값으로 설정
...
const handleReservationDateChange = (date: string) => {
  ...
  // 부모(SpaceReservationPage)에 알려서 selectedDate 상태 업데이트
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!values.reservationDate) {
      ...
      // 날짜 범위 검증 (오늘부터 다음 주차 토요일까지)
      const today = getTodayString();
      const maxDate = getNextWeekSaturday();
      if (values.reservationDate < today) {
        newErrors.reservationDate = '과거 날짜는 선택할 수 없습니다.';
      } else if (values.reservationDate > maxDate) {
        newErrors.reservationDate = `예약 가능한 날짜는 ${maxDate}까지입니다.`;
      }
    }
    ...
    if (values.totalUsers < 1) {
      newErrors.totalUsers = '인원 수는 1명 이상이어야 합니다.';
    }
    if (values.totalUsers > space.capacity) {
      newErrors.totalUsers = `인원 수는 최대 ${space.capacity}명까지 가능합니다.`;
    }
    if (!values.allUsers || values.allUsers.trim() === '') {
      newErrors.allUsers = '전체 이용자 정보를 입력해 주세요.';
    }
          ...
          <form onSubmit={handleSubmit} className="space-reservation-form__form">
            <div className="space-reservation-form__sections">
              ...
              <AllUsersInfo
                ...
                error={{
                  allUsers: errors.allUsers,
                  totalUsers: errors.totalUsers,
                }}
      ...
      {onCancel && (
        <div className="space-reservation-form__cancel">
          <button
            type="button"
            onClick={onCancel}
            className="space-reservation-form__cancel-button"
          >
            돌아가기
          </button>
        </div>
